...
class Runner_SMAC_QMIX:
    def __init__(self, args, number, seed):
        ...
        self.env.visions = np.ones(self.args.N)*self.args.sight_range
...
class Runner_SMAC_QMIX_IQL:
    ...
    def run(self, ):
        ...
        self.captain_action_history = np.array(self.captain_action_history, dtpye = np.float16)
    ...
    def run_episode_smac(self, evaluate=False):
        ...
        for episode_step in range(self.args.episode_limit):
            ...
            captain_reward = r + (9*self.args.N-self.env.visions.sum())/self.kill_sight_reward_ratio/self.args.N  # captain reward 
